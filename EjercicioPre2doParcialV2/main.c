...
int main()
{
    ...
    if(parser_parseEmpleados("data.csv",listaEmpleados)==1)
    {
        ...
        if(generarArchivoSueldos("sueldos.csv",listaEmpleados)==1)
        {
         ...
         al_map(listaEmpleados, Empleado_imprimirEmpleados);
         al_sort(listaEmpleados,Empleado_compararPorNombre,0);
         al_map(listaEmpleados, Empleado_imprimirEmpleados);
         al_sort(listaEmpleados, Empleado_compararPorHorasTrabajadas,0);
         al_map(listaEmpleados, Empleado_imprimirEmpleados);
    }
    else
